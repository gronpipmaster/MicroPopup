(function( $ ){
	var methods = {
		init : function ( options ){
			...
			popup.settings.image = (methods.data.find('img').length > 0) ? true : false;
			if( ! popup.settings.image ){
				methods.data.unbind('click').bind('click',function(){
					var url = this.href;
					$.get( url, function( data ){
							methods.settings.image = false;
							methods.open( data, url );
							if( popup.settings.callback != 'function (){}' )
								popup.settings.callback();
					});
					return false;
				});
			} else {
		...
		slidingBind : function(){
			...
			methods.container.find( '.controls a' ).unbind('click').bind('click', function(){
				methods.slidingAction( $( this ) );
				return false;
			});
		...
		slidingAction : function ( link ) {
			...
			this.settings.prev  = $( '[data-index=' + ( parseInt( link.attr('data-index'), 10) - 1) + ']' );
			this.settings.next  = $( '[data-index=' + ( parseInt( link.attr('data-index'), 10) + 1) + ']' );
